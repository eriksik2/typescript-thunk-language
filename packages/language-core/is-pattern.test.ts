...
describe("is pattern parse", () => {
  test("x is Err is IsExpression", () => {
    const ast = parseThunkSource(`const b = x is Err\n`);
  ...
  test("x is Err: infer e parses binding", () => {
    const ast = parseThunkSource(`const b = x is Err: infer e\n`);
    const init = (
      ast.statements[0] as {
        initializer: {
          kind: string;
  ...
  test("ready && x is Err: infer e is And of Is", () => {
    const ast = parseThunkSource(`const b = ready && x is Err: infer e\n`);
    const init = (
      ast.statements[0] as {
        initializer: {
          kind: string;
  ...
  test(".is property is not pattern is", () => {
    const ast = parseThunkSource(`const b = obj.is\n`);
    const init = (ast.statements[0] as { initializer: { kind: string } })
      .initializer;
...
describe("is pattern lower", () => {
  test("boolean is without binding", () => {
    const lowered = lowerThunkSource(`import { Err } from "@thunk/runtime"
const x = Err("e")
const b = x is Err
`);
  ...
  test("if (x is Err: infer e) binds e inside thunk", () => {
    const lowered = lowerThunkSource(`import { Err, Ok, type Result } from "@thunk/runtime"
...
`);
  ...
  test("if (ready && r is Ok: infer n) nests flow inside thunk", () => {
    const lowered = lowerThunkSource(`import { Ok, type Result } from "@thunk/runtime"
...
`);
  ...
  test("binding is in value position throws at lower", () => {
    expect(() =>
      lowerThunkSource(`import { Err } from "@thunk/runtime"
const e = Err("x")
