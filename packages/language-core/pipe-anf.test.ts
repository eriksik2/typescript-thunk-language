...
describe("pipe parse", () => {
  test("x | f is PipeExpression", () => {
    const ast = parseThunkSource(withPrelude(`const y = x | f\n`));
  ...
  test("run tx | f is Run wrapping Pipe", () => {
    const ast = parseThunkSource(withPrelude(`const v = run tx | flatten(1)\n`));
    const init = (
      bodyStmts(ast)[0] as {
        initializer: {
          kind: string;
  ...
  test("a | b | c is left-associative", () => {
    const ast = parseThunkSource(withPrelude(`const y = a | b | c\n`));
    const init = (
      bodyStmts(ast)[0] as {
        initializer: {
          kind: string;
  ...
  test("(run tx) | f embeds Run inside Ts left of Pipe", () => {
    const ast = parseThunkSource(withPrelude(`const y = (run tx) | f\n`));
    const init = (
      bodyStmts(ast)[0] as {
        initializer: {
          kind: string;
