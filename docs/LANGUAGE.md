...
The language may use continuation-based and functional representations internally, but those are implementation details.

The initial language is centered around:

1. thunks;
2. explicit execution with `run`;
3. compiler-generated continuations;
4. pipe syntax;
...
runs `source`, receives its value, and then runs the thunk returned by the continuation.

`bind` is the semantic operation generated by `run` inside a thunk.

The continuation represents the remainder of the enclosing thunk body.
...
is generated from the remainder of the thunk body.
...
Each `run` introduces one `bind`.

The remainder of the body becomes the continuation.
...
2. split the thunk body into continuation regions;
3. preserve variable scope;
4. generate `bind` operations;
5. wrap deferred execution in `defer`;
6. convert final returns into `succeed`;
7. preserve control-flow semantics.
...
These restrictions simplify the first continuation transformation.

The language can later support more positions through ANF-style normalization or a more complete control-flow transformation.
...
The generated continuation captures `prefix`.

The compiler should use ordinary JavaScript closure semantics initially.
...
Prefer correct, readable `bind`/`defer` output over supporting every JavaScript construct immediately.
...
The initial runtime may be recursive.

A later implementation may use an iterative continuation stack to avoid JavaScript stack growth.
